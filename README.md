...
- **关闭播放**：顶栏右侧「✕」按钮结束播放并收起播放器区（视频流与播放器销毁、标题重置、播放历史与进度保留），重新点击项目即可再次播放
- **ArtPlayer 内核**：主题色 `#2563eb`，默认音量 100%，支持播放倍速、画中画、截图、镜像翻转、画面比例等
- **HLS 流媒体**：hls.js 直接播放 `.m3u8`，多线路可切换（自动记忆上次线路）
- **线路自动切换**：当前线路出现致命网络错误（源站连接超时 / 404 / 失效）时，自动切换至下一条线路并继续播放（自动记忆本次选择）；全部线路均失败则停止并提示「所有线路均不可用，请稍后重试」
